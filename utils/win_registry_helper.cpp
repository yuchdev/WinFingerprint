...
#include "win_system_information.h"
#include "win_registry_helper.h"
#include <map>
#include <string>
#include <locale>
...
RegistryKey::RegistryKey(const std::string& key, bool read_only/* = false*/) : hkey_(new HKEY_holder)
{
    ...
    REGSAM samDesired;
    if (is_x32_application_on_x64()) samDesired = KEY_WOW64_64KEY;
    else samDesired = KEY_WOW64_32KEY;
...
void RegistryKey::set_binary_value(const string &key_name, const string &value)
{
    const std::string str_value(value);
    WinErrorChecker::last_error_throw_retcode(RegSetValueExA(hkey_->key,    // Root key
        key_name.c_str(),
...
std::pair<bool, string> RegistryKey::restore_value(const string &file_path, const std::string& key_name) const
{
    auto result = std::make_pair(true, std::string());

    if (!enable_restore_privilege()) {
        result.first = false;
        result.second = "Can not get 'backup' privilages!";
        return result;
    }

    auto err_str = WinErrorChecker::last_error_nothrow_retcode(RegRestoreKeyA(hkey_->key, (file_path + key_name).data(), NULL));
    if (!err_str.empty()) {
        result.first = false;
        result.second = err_str;
        return result;
    }
...
void RegistryKey::create_key(const string &key_name) const
{
    WinErrorChecker::last_error_throw_retcode(RegCreateKeyExA(hkey_->key,
                                                  key_name.data(),
                                                  0,
                                                  NULL,
...
void RegistryKey::delete_key(const string &key_name) const
{
    WinErrorChecker::last_error_throw_retcode(RegDeleteKeyExA(hkey_->key,
                                                              key_name.data(),
                                                              KEY_ALL_ACCESS,
                                                              NULL));
}

void RegistryKey::delete_value(const string &value_name)
{
    WinErrorChecker::last_error_throw_retcode(RegDeleteValueA(hkey_->key, value_name.data()));
}

std::pair<bool, string> helpers::RegistryKey::save_value(const std::string &file_path, const std::string& key_name) const
{
    auto result = std::make_pair(true, std::string());

    if (!enable_backup_privilege()) {
        result.first = false;
        result.second = "Can't get 'backup' privilages!";
        return result;
    }

    if (File::file_exist((file_path + key_name))) {
        File::remove_file((file_path + key_name));
    }

    auto str_err = WinErrorChecker::last_error_nothrow_retcode(RegSaveKeyA(hkey_->key,
                   (file_path + key_name).data(),
                   NULL));

    if (!str_err.empty()) {
        result.first = false;
        result.second = str_err;
        return result;
    }
    return result;
}
...
std::string RegistryKey::get_string_value(const std::string& key) const
{
    ...
    if (value_type != REG_SZ) {
        throw std::runtime_error("[" + key + "]: you trying to read is not a null-terminated string");
    }

    return string(buffer);
}

std::string RegistryKey::get_binary_value(const string &key) const
{
    char buffer[4096] = { };
    ...
    if (value_type != REG_BINARY) {
        throw std::runtime_error("[" + key + "]: value you trying to read is not a binary array!");
    }

    std::string ret(buffer, ret_buffer_size);

    if (ret.empty()) {
        throw std::runtime_error("[" + key + "]: value array is empty!");
    }
