...
bool RegistryKey::set_binary_value(const string &key_name, const std::vector<uint8_t>& array)
{
    return WinErrorChecker::retbool_nothrow_retcode(RegSetValueExA(hkey_->key,      // Root key
                                                                   key_name.c_str(),                                                           // param name
                                                                   0,                                                                          // reserved, == 0
                                                                   REG_BINARY,                                                                 // value type - binary array
                                                                   reinterpret_cast<const BYTE*>(array.data()),                                // buffer where value stored
                                                                   static_cast<DWORD>(array.size() + 1)));                                     // buffer size
}
...
std::pair<std::vector<uint8_t>, bool> RegistryKey::get_binary_value(const string &key) const
{
    std::pair<std::vector<uint8_t>, bool> result = std::make_pair(std::vector<uint8_t>(), true);
    char buffer[5192] = { 0 };
    ...
    result.first.reserve(static_cast<std::size_t>(ret_buffer_size));

    for (DWORD i(0); i < ret_buffer_size; ++i)
        result.first.push_back(static_cast<uint8_t>(buffer[i]));
