...
std::map<unsigned short, std::string> helpers::NativeSystemInformation::cpu_architectures_ = {
...
bool helpers::is_x32_application_on_x64()
{
...
void NativeSystemInformation::get_system_version()
{
    ...
    last_error_ = (WinErrorChecker::last_error_nothrow_boolean(GetVersionEx(reinterpret_cast<OSVERSIONINFO*>(&info))));
    ...
    std::stringstream s;
    s << major_version() << '.' << minor_version() << '.' << build_number();
    os_version_ = std::move(s.str());
...
std::string NativeSystemInformation::get_last_error() const
{
    return last_error_;
}
